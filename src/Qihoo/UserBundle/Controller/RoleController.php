...
namespace Qihoo\UserBundle\Controller;
...
use Qihoo\UserBundle\Entity\Role;
use Qihoo\UserBundle\Entity\Block;
...
class RoleController extends Controller
{
     ...
     * @Template("QihooUserBundle:Role:list.html.twig")
     */
    public function listAction()
    {
        ...
        list($total, $data) = $em->getRepository('QihooUserBundle:Role')->getList($start, $limit);
     ...
     * @Template("QihooUserBundle:Role:query_result.html.twig")
     */
    public function queryAction()
    {
        ...
        $em = $this->getDoctrine()->getManager();
        list($total, $data) = $em->getRepository('QihooUserBundle:Role')->query($start, $limit, $queryParams);
     ...
     * @Template("QihooUserBundle:Role:add.html.twig")
     */
    public function addAction()
    {
        ...
        try {
            ...
            if ($id) {
                $Role = $em->getRepository('QihooUserBundle:Role')->findRoleByRoleId($id);
            } else {
                $Role = array();
            }
            ...
            if ($confirm) {
                ...
                if (!$id) {
                    $result = $em->getRepository('QihooUserBundle:Role')->findRoleByRoleName($name);
                    if (!$result) {
                        $role_id = $em->getRepository('QihooUserBundle:Role')->addRole($role_data_create);
                    } else {
                    ...
                    $em->getRepository('QihooUserBundle:Role')->updateRole($role_data_update, array('id' => $id));
                }

                $post_route = (array)$request->get('route');

                foreach (array_diff($post_route, $exist_route) as $route_id) {
                    $em->getRepository('QihooUserBundle:Role')->addAccess(array('role_id' => $role_id, 'route_id' => $route_id));
                }

                foreach (array_diff($exist_route, $post_route) as $route_id) {
                    $em->getRepository('QihooUserBundle:Role')->deleteAccess(array('role_id' => $role_id, 'route_id' => $route_id));
                }
    ...
    public function deleteAction()
    {
        ...
        $id      = $request->get('id');
        $em      = $this->getDoctrine()->getEntityManager();
        $Block   = $em->getRepository('QihooUserBundle:Role')->find($id);
    ...
    {
        $em     = $this->getDoctrine()->getEntityManager();
        $result = $em->getRepository('QihooUserBundle:Path')->getMenu();
