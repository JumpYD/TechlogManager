...
class UserRepository extends EntityRepository
{
    public function getList($start, $limit, $queryParams = array())
    {
        ...
        $SQL_TOTAL = "SELECT COUNT(*) AS total FROM qihoo_user u";
    ...
    public function findUserRole($username)
    {
        ...
        $SQL = "SELECT role_id FROM qihoo_user WHERE username='" . $username . "'";
    ...
    public function getUserByName($username)
    {
        ...
        $DQL    = "SELECT u FROM QihooUserBundle:User u WHERE u.username=:username";
    ...
    public function getUserList()
    {
        $em = $this->getEntityManager();
        $em->getConnection()->executeQuery("SET NAMES utf8");
        $DQL = "SELECT u FROM QihooUserBundle:User u";
        $query = $em->createQuery($DQL);
        $result = $query->getResult();
    ...
    public function isAvailableUser($username)
    {
        ...
        $sql = "select distinct(username) as username from qihoo_user";
    ...
    public function isRightPassword($username, $password)
    {
        $em = $this->getEntityManager();
        $store = $em->getConnection();
        $sql = "select password from qihoo_user_auth where username=:username";
        $sth = $store->prepare($sql);
