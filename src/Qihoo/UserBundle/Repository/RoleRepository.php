...
namespace Qihoo\UserBundle\Repository;
...
class RoleRepository extends EntityRepository
{
    public function getList($start, $limit)
    {
        ...
        $DQL_TOTAL = "SELECT COUNT(a) FROM QihooUserBundle:Role a";
        $DQL = "SELECT a FROM QihooUserBundle:Role a ORDER BY a.createTime DESC";
    ...
    public function query($start, $limit, $params = array())
    {
        ...
        $SQL_TOTAL = "SELECT COUNT(DISTINCT r.id) AS total FROM qihoo_role r
                LEFT JOIN qihoo_access a
                ON r.id = a.role_id
        ";

        $SQL = "SELECT DISTINCT r.* FROM qihoo_role r
                LEFT JOIN qihoo_access a
                ON r.id = a.role_id
        ";
    ...
    public function getRoleList()
    {
        ...
        $SQL = "SELECT id, name AS roleName FROM qihoo_role";
    ...
    public function findRoleByRoleId($roleId)
    {
        ...
        $SQL = "SELECT r.*, a.route_id FROM qihoo_role r
                LEFT JOIN qihoo_access a
                ON r.id = a.role_id
                WHERE r.id='" . $roleId . "'
        ";
    ...
    public function findRoleByRoleName($roleName)
    {
        ...
        $SQL = "SELECT r.id, a.route_id FROM qihoo_role r
                LEFT JOIN qihoo_access a
                ON r.id = a.role_id
    ...
    public function updateRole($data, $identifier)
    {
        ...
        $conn->update('qihoo_role', $data, $identifier);
        $role_id = $conn->lastInsertId();
        $conn->close();
    ...
    public function findRoleAccessPath($roleIds)
    {
        ...
        $SQL = "SELECT route_id FROM qihoo_access WHERE role_id IN(" . implode(',', $roleIds) . ")";
        $result = $conn->fetchAll($SQL);
        $conn->close();
	...
	{
		$em = $this->getEntityManager();
		$dql = "SELECT role FROM QihooUserBundle:Role role, QihooUserBundle:User user WHERE role.id=user.roleId";
