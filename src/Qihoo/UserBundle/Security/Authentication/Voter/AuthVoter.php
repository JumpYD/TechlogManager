...
namespace Qihoo\UserBundle\Security\Authentication\Voter;

use Symfony\Component\DependencyInjection\ContainerInterface;
use Symfony\Component\Security\Core\Authorization\Voter\VoterInterface;
use Symfony\Component\Security\Core\Authentication\Token\TokenInterface;
use Symfony\Component\HttpFoundation\Session\Session;
use Qihoo\UserBundle\Security\Token\QihooUserToken;
use Qihoo\UserBundle\Entity\OptLog;
...
class AuthVoter implements VoterInterface
{
    ...
    {
        return $class === 'Qihoo\UserBundle\Security\Token\QihooUserToken';
    }
    ...
    public function vote ( TokenInterface $token, $object, array $attributes )
    {
        ...
        {
            $access_route = $this->container->get ( 'doctrine' )->getRepository ( 'QihooUserBundle:Path' )->getMenu ();
    ...
    private function checkAuth ( $username, $route, &$access_route )
    {
        ...
        {
            $result = $this->container->get ( 'doctrine' )->getRepository ( 'QihooUserBundle:User' )->findUserRole ( $username );
        ...
        {
            $roleRoutes[ $username ] = $this->container->get ( 'doctrine' )->getRepository ( 'QihooUserBundle:Role' )->findRoleAccessPath ( $user[ $username ] );
        }
        ...
        $result = $this->container->get ( 'doctrine' )->getRepository ( 'QihooUserBundle:Path' )->getMenu ();
        foreach ( $result as $value )
        {
        ...
        if ( $route != 'qihoo_homepage' && !in_array ( $route, $routeList ) )
        {
            return VoterInterface::ACCESS_DENIED;
        }
