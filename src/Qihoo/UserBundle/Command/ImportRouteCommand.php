<?php
namespace Qihoo\UserBundle\Command;
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        while ($data = fgetcsv($handle, 1000, ',')) {
            ...
            if (count($data) == 3) {
                ...
                //判断一级目录是否存在
                $entity = $em->getRepository('QihooUserBundle:Path')->findOneBy(array('name' => $category1_name));
                ...
                // 判断路由是不是存在
                $entitys = $em->getRepository('QihooUserBundle:Path')->findBy(array('route' => $route));
            ...
            if (count($data) == 4) {
                ...
                //判断一级目录是否存在
                $entitys = $em->getRepository('QihooUserBundle:Path')->findBy(array('name' => $category1_name));
                if (count($entitys) != 1) {
                ...
                //判断二级目录是否存在
                $entitys = $em->getRepository('QihooUserBundle:Path')->findBy(array('name' => $category2_name));
                if (count($entitys) != 1) {
